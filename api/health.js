...
module.exports = async (req, res) => {
  const base = {
    ...
    catalogStore: store.durable ? `durable — found via ${store.via}` : 'in-memory (published menus reset on cold start)',
    /* names only, never values — so a failed KV hookup is diagnosable without leaking secrets */
    storageEnvSeen: Object.keys(process.env)
      .filter(k => /^(KV_|UPSTASH_|STORAGE_|REDIS_)/.test(k) || /_REST_(API_)?(URL|TOKEN)$/.test(k))
      .sort(),
    note: KEY
